...
{
    debug_log_add("Scribble: WARNING! Event name \"" + _name + "\" has already been defined as a colour");
    exit;
}

if (ds_map_exists(global.__scribble_effects, _name))
{
    debug_log_add("Scribble: WARNING! Event name \"" + _name + "\" has already been defined as an effect");
    exit;
}
...
{
    debug_log_add("Scribble: WARNING! Overwriting event [" + _name + "] tied to script " + script_get_name(_old_script) + "()");
}

global.__scribble_autotype_events[? _name] = _script;
if (SCRIBBLE_VERBOSE) debug_log_add("Scribble: Tying event [" + _name + "] to script " + script_get_name(_script) + "()");