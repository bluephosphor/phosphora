switch(state){
	case inv_state.inv: ///////////////INVENTORY STATE
		...
		var i = 0; var xx = 0; var yy = 0; repeat(max_items){
			...
			//draw items
			draw_sprite(spr_items,inventory[# ITEM_ID,i],dx,dy);
			//draw cells
			var c = c_white;
			...
			if (inventory[# COUNT,i] > 1) {
				draw_item_count(dx,dy,inventory[# COUNT, i]);
			}
			//draw_effects_data
			if (i == menu_index) eff_coords = new vec2(dx,dy+cell_size);
			i++;
			xx++;
			if (xx >= items_width){
				yy++;
		...
		nine_slice_box_s(spr_9slice1,desc_x,desc_y,desc_width,desc_height,global.menu_color);
			...
			c = item_info[# COLOR, selected_item];
			var name = "[" + item_info[# ITEM,selected_item] + "]";
			var desc = item_info[# INFO,selected_item];
		...
		nine_slice_box_s(spr_9slice1,info_x,info_y,info_width,info_y+info_height,global.menu_color);
			...
			i = 1; repeat (array_length(has_rift)){
	...
	case inv_state.chest: ///////////////CHEST STATE
		...
		var i = 0; var xx = 0; var yy = 0; repeat(max_items){
			...
			//draw_effects_data
			if (i == menu_index and selecting_grid == inventory) eff_coords = new vec2(dx,dy+cell_size);
			i++;
			xx++;
			if (xx >= items_width){
				yy++;
	...
	case inv_state.hotbar: ///////////////////HOTBAR STATE
		var xx = 0; repeat(cell_count){
			...
			//draw items
			draw_sprite(spr_items,inventory[# ITEM_ID,xx],x_origin+offset,y_origin);
			//draw counts
			if (inventory[# COUNT,xx] > 1) {
				draw_item_count(x_origin+offset,y_origin,inventory[# COUNT, xx]);
			}
			xx++;
		}
		break;
